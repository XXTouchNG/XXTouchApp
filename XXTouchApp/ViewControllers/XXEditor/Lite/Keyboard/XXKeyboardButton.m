...
#import "XXKeyboardButtonView.h"
#import "UIImage+ColoredImage.h"

@interface XXKeyboardButton ()

@property (nonatomic, strong) UILabel *inputLabel;
@property (nonatomic, strong) XXKeyboardButtonView *buttonView;
@property (nonatomic, assign) XXKeyboardButtonPosition position;
...
@implementation XXKeyboardButton
...
- (void)commonInit
{
    ...
    UIFont *f = [UIFont systemFontOfSize:_fontSize];
    ...
    l.text = @"3";
    l.font = [UIFont systemFontOfSize:_bigFontSize];
...
- (void)setInput:(NSString *)input
{
    ...
    for (int i = 0; i < MIN(input.length, 5); i++) {
        UILabel *currentLabel = [self.labels objectAtIndex:i];
...
- (void)setStyle:(XXKeyboardButtonStyle)style
{
...
{
    NSAssert([textInput conformsToProtocol:@protocol(UITextInput)], @"<XXKeyboardButton> The text input object must conform to the UITextInput protocol!");
...
- (void)selectLabel:(int)idx {
    if (idx == -1) {
        ...
        self.output = self.labels[idx].text;
...
{
    switch (_style) {
        case XXKeyboardButtonStylePhone:
            _keyCornerRadius = 4.f;
            break;
            
        case XXKeyboardButtonStyleTablet:
...
- (void)updateButtonPosition
{
    ...
    if (leftPadding >= minimumClearance && rightPadding >= minimumClearance) {
        self.position = XXKeyboardButtonPositionInner;
    } else if (leftPadding > rightPadding) {
        self.position = XXKeyboardButtonPositionLeft;
    } else {
        self.position = XXKeyboardButtonPositionRight;
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_trackPoint) {
        if (fabs([_firstTapDate timeIntervalSinceNow]) < TIME_INTERVAL_FOR_DOUBLE_TAP) {
            _selecting = YES;
        } else {
            _selecting = NO;
        }
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_style == XXKeyboardButtonStyleTablet && self.state == UIControlStateHighlighted) {
