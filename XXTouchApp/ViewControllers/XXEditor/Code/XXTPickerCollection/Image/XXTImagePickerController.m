...
- (void)initControls {
    // side buttons
    _btUp.backgroundColor = [[XXTPickerHelper sharedInstance] frontColor];
    _btDown.backgroundColor = [[XXTPickerHelper sharedInstance] frontColor];
...
- (void)initBottomMenu {
    _vBottomMenu.backgroundColor = [[XXTPickerHelper sharedInstance] frontColor];
    ...
    if (_nMaxCount == XXT_NO_LIMIT_SELECT) {
        _lbSelectCount.text = NSLocalizedStringFromTableInBundle(@"(0)", nil, [XXTPickerHelper bundle], nil);
        ...
        if (_nResultType == XXT_PICKER_RESULT_UIIMAGE) {
            ...
            if (resultImage) {
                ...
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"Error", nil, [XXTPickerHelper bundle], nil)
                                                                    message:NSLocalizedStringFromTableInBundle(@"Please download this image from iCloud.", nil, [XXTPickerHelper bundle], nil)
                                                                   delegate:nil
...
- (void)showPhotosInGroup:(NSInteger)nIndex {
    if (_nMaxCount == XXT_NO_LIMIT_SELECT) {
        ...
        _lbSelectCount.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"(0/%d)", nil, [XXTPickerHelper bundle], nil), (int) _nMaxCount];
    }
