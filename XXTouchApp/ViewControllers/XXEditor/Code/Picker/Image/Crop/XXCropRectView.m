...
#import "XXResizeControl.h"

@interface XXCropRectView () <XXResizeControlViewDelegate>

@property (nonatomic) XXResizeControl *topLeftCornerView;
@property (nonatomic) XXResizeControl *topRightCornerView;
@property (nonatomic) XXResizeControl *bottomLeftCornerView;
@property (nonatomic) XXResizeControl *bottomRightCornerView;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        imageView.image = [[UIImage imageNamed:@"picker-border"] resizableImageWithCapInsets:UIEdgeInsetsMake(23.0f, 23.0f, 23.0f, 23.0f)];
        [self addSubview:imageView];
        
        self.topLeftCornerView = [[XXResizeControl alloc] init];
        self.topLeftCornerView.delegate = self;
        [self addSubview:self.topLeftCornerView];
        
        self.topRightCornerView = [[XXResizeControl alloc] init];
        self.topRightCornerView.delegate = self;
        [self addSubview:self.topRightCornerView];
        
        self.bottomLeftCornerView = [[XXResizeControl alloc] init];
        self.bottomLeftCornerView.delegate = self;
        [self addSubview:self.bottomLeftCornerView];
        
        self.bottomRightCornerView = [[XXResizeControl alloc] init];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    for (UIView *subview in subviews) {
        if ([subview isKindOfClass:[XXResizeControl class]]) {
...
- (void)resizeControlViewDidBeginResizing:(XXResizeControl *)resizeControlView
{
    ...
    if (resizeControlView == self.topLeftCornerView) {
        _resizeControlPosition = kXXResizeControlPositionTopLeft;
    } else if (resizeControlView == self.topRightCornerView) {
        _resizeControlPosition = kXXResizeControlPositionTopRight;
    } else if (resizeControlView == self.bottomLeftCornerView) {
        _resizeControlPosition = kXXResizeControlPositionBottomLeft;
    } else if (resizeControlView == self.bottomRightCornerView) {
        _resizeControlPosition = kXXResizeControlPositionBottomRight;
    }
...
- (CGRect)cropRectMakeWithResizeControlView:(XXResizeControl *)resizeControlView
{
