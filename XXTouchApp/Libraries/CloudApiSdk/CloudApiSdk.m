...
+ (instancetype)instance {
    ...
    static CloudApiSdk *httpUtil = nil;
    dispatch_once(&onceToken, ^{
        httpUtil = [CloudApiSdk new];
    });
    return httpUtil;
}
...
{
    NSURLRequest * request = [CloudApiSdk buildRequest:httpSchema
                                             method:CLOUDAPI_GET
                                               host:host
                                               path:path
...
@implementation CloudApiSdk (private)
...
+ (NSURLRequest *) buildRequest:protocol
     ...
     */
    NSString * pathWithParam = [CloudApiSdk combinePathParam: path pathParams:pathParams];
    NSString * queryString = [CloudApiSdk buildParamsString:queryParams];
    /**
     *  拼接URL
     ...
     */
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString: url] cachePolicy:[[AppConfiguration instance] APP_CACHE_POLICY] timeoutInterval:[[AppConfiguration instance] APP_CONNECTION_TIMEOUT]];
    ...
    [headerParams setObject:[[AppConfiguration instance] APP_KEY] forKey:CLOUDAPI_X_CA_KEY];
     ...
     */
    if(nil != formParams){
        [request setHTTPBody: [[CloudApiSdk buildParamsString:formParams] dataUsingEncoding:NSUTF8StringEncoding]];
    }
     ...
     */
        ...
        [headerParams setObject: [CloudApiSdk md5:body] forKey:CLOUDAPI_HTTP_HEADER_CONTENT_MD5];
