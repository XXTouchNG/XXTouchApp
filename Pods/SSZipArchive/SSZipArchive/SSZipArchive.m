...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    [self createZipFileAtPath:path withFilesAtPaths:paths withPassword:password delegate:nil];
}
                   ...
                   delegate:(id <SSZipArchiveDelegate>)delegate
{
    ...
    if ([zipArchive open]) {
        ...
        for (NSString *filePath in paths) {
            ...
            {
                NSString *relativePath = [filePath stringByReplacingOccurrencesOfString:directoryPath withString:@""];
                if (isDir) {
                    [zipArchive writeFolderAtPath:filePath withFolderName:relativePath withPassword:password];
                } else {
                    [zipArchive writeFileAtPath:filePath withFileName:relativePath withPassword:password];
                }
            }
        }
    ...
    if ([delegate respondsToSelector:@selector(zipArchiveDidCreatedArchiveAtPath:)]) {
        [delegate zipArchiveDidCreatedArchiveAtPath:path];
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    return [self createZipFileAtPath:path
                 withContentsOfItems:@[directoryPath]
                 keepParentDirectory:keepParentDirectory
                        withPassword:password
                            delegate:delegate];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path
        withContentsOfItems:(NSArray <NSString *> *)items
        keepParentDirectory:(BOOL)keepParentDirectory
               withPassword:(nullable NSString *)password
                   delegate:(nullable id <SSZipArchiveDelegate>)delegate {
    BOOL success = NO;
    ...
    if ([zipArchive open]) {
        ...
        for (NSString *itemPath in items) {
            BOOL kDir;
            BOOL exists = [fileManager fileExistsAtPath:itemPath isDirectory:&kDir];
            NSString *tFileName = [itemPath lastPathComponent];
            if (!exists) continue;
            if (!kDir) {
                [zipArchive writeFileAtPath:itemPath withFileName:tFileName withPassword:password];
            }
            else
            {
                if ([fileManager subpathsOfDirectoryAtPath:itemPath error:nil].count == 0)
                {
                    [zipArchive writeFolderAtPath:itemPath withFolderName:tFileName withPassword:password];
                } else {
                    NSDirectoryEnumerator *dirEnumerator = [fileManager enumeratorAtPath:itemPath];
                    NSString *objectName = nil;
                    while ((objectName = [dirEnumerator nextObject])) {
                        BOOL isDir;
                        NSString *fullFilePath = [itemPath stringByAppendingPathComponent:objectName];
                        [fileManager fileExistsAtPath:fullFilePath isDirectory:&isDir];
                        
                        NSString *fileName = [[itemPath lastPathComponent] stringByAppendingPathComponent:objectName];
                        ...
                        {
                            if ([fileManager subpathsOfDirectoryAtPath:fullFilePath error:nil].count == 0)
                            {
