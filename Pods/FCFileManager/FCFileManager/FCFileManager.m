...
+(BOOL)createFileAtPath:(NSString *)path withContent:(NSObject *)content overwrite:(BOOL)overwrite error:(NSError **)error
{
    if(![self existsItemAtPath:path] || (overwrite && [self removeItemAtPath:path error:error] && [self isNotError:error]))
    {
        ...
        *error = [NSError errorWithDomain:NSCocoaErrorDomain code:0 userInfo:@{ NSLocalizedDescriptionKey:[NSString stringWithFormat:NSLocalizedString(@"File \"%@\" already exists.", nil), [path lastPathComponent]] }];
        return NO;
    }
}
...
+(NSString *)sizeFormattedOfDirectoryAtPath:(NSString *)path cancelFlag:(int *)cancelFlag
{
    return [self sizeFormattedOfDirectoryAtPath:path error:nil cancelFlag:cancelFlag];
}


+(NSString *)sizeFormattedOfDirectoryAtPath:(NSString *)path error:(NSError **)error cancelFlag:(int *)cancelFlag
{
    NSNumber *size = [self sizeOfDirectoryAtPath:path error:error cancelFlag:cancelFlag];
...
+(NSNumber *)sizeOfDirectoryAtPath:(NSString *)path cancelFlag:(int *)cancelFlag
{
    return [self sizeOfDirectoryAtPath:path error:nil cancelFlag:(int *)cancelFlag];
}
